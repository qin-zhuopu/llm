...
> **核心结论**：真正导致质量暴跌的是「弱模型 + 截断上下文」，而不是「子代理」这一形式本身。
> 因此阶段④既可由当前会话 Agent 处理，也可交给具备完整文件工具的**强能力子代理**端到端处理。
> 无论谁做，都必须守住三条核心约束：**读完整 MD（禁止截断）+ 禁止 glm-4.5-air 那种弱模型老模式 + 每批 10-15 个跑 validate.py + test_all.py**。
...
阶段④允许交给**具备完整文件工具的强能力子代理**端到端处理（读 md + schema、自己理解内容、自主研究补充、自己写 YAML）。前提是子代理必须守住核心约束：**读完整 MD（禁止截断）+ 禁止 glm-4.5-air 那种弱模型老模式 + 每批 10-15 个跑 validate.py + test_all.py**。

orchestrator 传递任务时的规则：

1. **只传文件路径列表**，不嵌入 md 原文
2. **子代理自己用 `read_file` 读取** md 文件**完整内容**（禁止截断）
